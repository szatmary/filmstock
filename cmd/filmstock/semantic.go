...
func (s *server) hitsToResults(hits []Hit) []filmstock.UnifiedResult {
	out := make([]filmstock.UnifiedResult, 0, len(hits))
	for _, h := range hits {
		...
		out = append(out, filmstock.UnifiedResult{
			Type:     "movie",
			Title:    filmstock.CleanTitle(title),
			Subtitle: yr,
			Link:     fmt.Sprintf("/movie?id=%d", h.PageID),
			Cover:    filmstock.FilePathURL(cover, 160),
			Score:    h.Score,
		})
	}
	return out
