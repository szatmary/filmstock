...
func buildHeader(m *filmstock.Movie, section string) string {
	var b strings.Builder
	b.WriteString(filmstock.CleanTitle(m.Title))
	if y := filmYear(m); y != "" {
		b.WriteString(" (" + y + ")")
	}
	if len(m.Director) > 0 {
...
func filmYear(m *filmstock.Movie) string {
...
func cmdChunk(args []string) {
	...
	var paths []string
	if err := filmstock.WalkRecords(*records, filmstock.KindText, func(p string) error {
	...
	for i := 0; i < *workers; i++ {
		...
		go func() {
			...
			for p := range jobs {
				...
				var mv filmstock.Movie
				hdrOK := !*noHeader &&
					filmstock.ReadRecordJSON(filmstock.RecordPath(*records, filmstock.KindMovie, int64(id), ".json.gz"), &mv) == nil
