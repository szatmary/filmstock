...
func extractRecords(d *dumpSet, outDir string, workers int, wantText bool, limit int) error {
	...
	for _, k := range []string{kindMovie, kindTelevision, kindPerson} {
		if err := os.MkdirAll(filepath.Join(outDir, k), 0o755); err != nil {
			return err
		}
	}
	if wantText {
		if err := os.MkdirAll(filepath.Join(outDir, kindText), 0o755); err != nil {
			return err
		}
	}
...
func (w *recordWriter) notePeople(groups ...[]Person) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *recordWriter) handleFilm(p Page) {
	...
	w.pool.put(recordPath(w.out, kindMovie, int64(p.ID), ".json.gz"), data)
	...
	if w.wantText {
		if td, err := encodeRecordText(fullPlainText(p.Text)); err != nil {
			...
			w.pool.put(recordPath(w.out, kindText, int64(p.ID), ".txt.gz"), td)
		}
	}
}
...
func (w *recordWriter) handleEvent(p Page) {
	...
	w.pool.put(recordPath(w.out, kindEvent, int64(p.ID), ".json.gz"), data)
	atomic.AddInt64(&w.events, 1)
	w.notePeople(e.Hosts)
}

func (w *recordWriter) handleSeries(s *TelevisionSeries) {
	data, err := encodeRecordJSON(s)
	if err != nil {
		w.fail(err)
		return
	}
	w.pool.put(recordPath(w.out, kindTelevision, int64(s.PageID), ".json.gz"), data)
...
func (w *recordWriter) flushPeople(cachePath string) (withQID, total int) {
	...
	for _, p := range w.people {
		...
		w.pool.put(recordPath(w.out, kindPerson, id, ".json.gz"), data)
		total++
	}
	return
}
