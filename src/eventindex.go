...
func cmdIndexEvents(args []string) {
	...
	dir := filepath.Join(*records, kindEvent)
	...
	err = walkRecords(*records, kindEvent, func(path string) error {
		e, err := readEventGz(path)
		if err != nil {
			return err
		}
...
func (s *server) handleAPIEvents(w http.ResponseWriter, r *http.Request) {
	res, err := searchEvents(r.Context(), s.db, r.URL.Query().Get("q"), 25)
	if err != nil {
...
func (s *server) handleEventPage(w http.ResponseWriter, r *http.Request) {
	...
	e, err := readEventGz(filepath.Join(s.eventsDir, relPath))
	if err != nil {
